...
def goals_report():
    ...
    try:
        data = pd.read_csv("Planering.csv")
        if data.empty:
...
def tools_per_categories_report():
    ...
    try:
        # Läs in CSV-filen
        df = pd.read_csv("data_categories.csv")
        ...
        st.info("Filen 'Planering.csv' eller 'data_categories.csv' hittades inte. Skapa ett nytt mål.")

    except pd.errors.EmptyDataError:
        st.info("Filen 'Planering.csv' eller 'data_categories.csv' är tom. Lägg till ett nytt mål.")
